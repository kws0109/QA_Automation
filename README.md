...
비개발자도 쉽게 사용할 수 있는 **시각적 노드 에디터**로 모바일 게임 테스트 시나리오를 작성하고, **최대 10대의 디바이스**에서 동시에 안정적으로 실행할 수 있습니다.

---

## 🤖 AI-Native 개발 방식

> 이 프로젝트는 **Claude Code(AI 코딩 에이전트)** 와 협업하여 설계부터 구현까지 전 과정을 개발했습니다.

| 항목 | 내용 |
|------|------|
| **개발 도구** | [Claude Code](https://claude.ai/code) — Anthropic AI 코딩 에이전트 |
| **개발 방식** | Vibe Coding — 자연어 요구사항 → 설계 → 코드 구현 전 과정 AI 협업 |
| **프로젝트 규모** | 4개 서브 프로젝트 (Backend / Frontend / Electron / Android) |
| **코드베이스** | 50+ 백엔드 서비스 파일, 40+ 프론트엔드 컴포넌트, TypeScript 100% 적용 |

게임 QA 현장에서 7년간 쌓은 도메인 지식을 바탕으로, 실제 현장의 비효율을 해결하기 위한 자동화 플랫폼을 AI와 함께 0에서 1로 만들었습니다. 비개발자(QA 담당자)가 코드 없이 테스트를 구성하고 실행할 수 있는 것이 핵심 목표입니다.
