 ...
 * 스크린샷 파일 제공
 */
router.get('/screenshots/:reportId/:deviceId/:filename', asyncHandler(async (req: Request<ScreenshotParams>, res: Response) => {
 ...
 */
router.get('/videos/:reportId/:filename', asyncHandler(async (req: Request<VideoParams>, res: Response) => {
 ...
 * 리포트 내보내기 (HTML/PDF)
 */
router.get('/:id/export/:format', asyncHandler(async (req: Request<ExportParams, unknown, unknown, ExportQuery>, res: Response) => {
  const { id, format } = req.params;
  const { screenshots, paper, orientation } = req.query;

  if (format !== 'html' && format !== 'pdf') {
    throw new BadRequestError('지원하지 않는 형식입니다. html 또는 pdf를 사용하세요.');
  }
 ...
 */
router.post('/:id/upload', asyncHandler(async (req: Request<IdParams, unknown, unknown, UploadQuery>, res: Response) => {
  const { id } = req.params;
