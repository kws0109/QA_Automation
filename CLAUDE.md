...
**이 도구는 현업에서 실제로 사용되는 QA 자동화 도구입니다.**

### 핵심 운영 특성
| 특성 | 설명 |
|------|------|
| **다중 사용자** | 여러 QA 담당자가 동시에 접속하여 사용 |
| **공유 디바이스** | 연결된 디바이스는 모든 사용자가 공유 |
| **동시 테스트** | 서로 다른 사용자가 동시에 테스트 실행 가능 |
| **리소스 경쟁** | 동일 디바이스에 여러 테스트 요청 발생 가능 |

### 설계 시 필수 고려사항
1. **디바이스 경쟁**: 사용 중인 디바이스에 테스트 요청 시 대기열 처리
2. **상태 공유**: 모든 사용자에게 실시간으로 디바이스/테스트 상태 동기화
3. **리소스 관리**: 테스트 대기열, 우선순위, 타임아웃 관리
4. **투명성**: 누가 어떤 디바이스를 사용 중인지 명확히 표시

### 부하 고려 기준 (코드 작성 시 최우선 규칙)
**다음 조건에서 네트워크 및 시스템 부하를 항상 고려해야 합니다:**

| 항목 | 기준값 | 비고 |
|------|--------|------|
| 동시 접속 사용자 | 다중 사용자 | |
| 연결 기기 수 | 20대 이상 | 목록 표시/관리 기준 |
| 동시 테스트 수 | **10대** | 안정적 동시 실행 보장 |

**동시 테스트 제한 이유:**
- OpenCV 이미지 매칭이 CPU 집약적 작업
- 세마포어로 동시 매칭 4개 제한 (`imageMatch.ts`)
- 10대 초과 시 대기열 지연 발생 가능

**코드 작성 시 체크리스트:**
- [ ] 폴링/업데이트 주기가 적절한가? (너무 짧으면 부하 증가)
- [ ] 불필요한 데이터 전송이 없는가?
- [ ] 클라이언트에서 처리 가능한 계산을 서버에서 하고 있지 않은가?
- [ ] 10대 동시 테스트 시나리오에서 병목이 없는가?
...
| 항목 | 기준 | 비고 |
|------|------|------|
| **디바이스 목록/UI** | 20대 | 목록 표시, 스크롤, 필터링 |
| **동시 테스트 실행** | **10대** | 안정적 동시 실행 보장 |

### 성능 평가 항목
| 항목 | 기준 |
|------|------|
| 네트워크 대역폭 | 20대 동시 스트리밍 시 예상 트래픽 계산 |
| 브라우저 메모리 | 20대 디바이스 UI 렌더링 시 메모리 사용량 |
| CPU 사용률 | 클라이언트/서버 모두 고려 |
| Appium 서버 부하 | 10대 동시 테스트 기준 |
| UI 응답성 | 20대 목록 스크롤, 필터링, 검색 성능 |

### 설계 원칙
1. **스케일링 고려**: 기능 추가 시 10대 동시 테스트에서 원활히 동작하는지 검토
