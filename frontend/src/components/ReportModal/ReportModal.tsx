// frontend/src/components/ReportModal/ReportModal.tsx
...
function ReportModal({ isOpen, onClose }: ReportModalProps) {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const res = await axios.get<{ data: ReportSummary[] }>(`${API_BASE}/api/reports`);
  ...
  const handleViewDetail = async (id: string) => {
    ...
    try {
      const res = await axios.get<{ data: ReportDetail }>(`${API_BASE}/api/reports/${id}`);
      setSelectedReport(res.data.data);
    } catch (err) {
      const error = err as Error;
      alert('리포트 조회 실패: ' + error.message);
    } finally {
